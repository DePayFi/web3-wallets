import { getProvider as getProvider$3, request as request$1, estimate as estimate$1 } from '@depay/web3-client-solana';
...
const submitInstructions = async ({ transaction, wallet })=> {
  ...
  const messageV0 = new TransactionMessage({
    ...
    transaction.alts ? await Promise.all(transaction.alts.map(async(alt)=>{
      return (await getProvider$3('solana')).getAddressLookupTable(new PublicKey(alt)).then((res) => res.value)
    })) : undefined);
  const transactionV0 = new VersionedTransaction(messageV0);
