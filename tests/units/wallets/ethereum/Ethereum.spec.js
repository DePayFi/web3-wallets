...
describe('depay-multichain-crypto-wallets', () => {
  ...
  it('provides a connect function', async () => {
    const requestMock = jest.fn().mockImplementation(async ()=> {
    ...
    expect(await wallet.connect()).toStrictEqual(['0x0000000000000000000000000000000000000000']);
  });

  it('provides an image', async () => {
    window.ethereum = {
    };
